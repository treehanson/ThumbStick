...
'
	...
	' DATABASE INIT
	'
	Dim cnnData					' Connection to database
	dim gblCommand

	' Database params (common)
	cnnCSentry_ConnectionTimeout = 60
	cnnCSentry_CommandTimeout = 60
	cnnCSentry_CursorLocation = 1
	bLive = false
	
	' 
	' SubDirectories (common)
	'
	gblBannersubdir = "banners"
	gblDisplaysubdir = "display"
	gblBillingsubdir = "billing"
	gblMoviefonedir = "c:\cinema\usmf3"
	
	'
	' CENTRAL test server
	'
	set pc = CreateObject("Wscript.Network") 	
	strServer = ucase(pc.ComputerName)
	set pc = nothing	
			...
			' All servers
			'
			'cnnCSentry_ConnectionString = "Driver={SQL Server};Network=dbmssocn;Server=CinemaDB;Database=Cinema;UID=cinema_user;PWD=hummer2;"
			cnnCSentry_ConnectionString = "Provider=sqloledb;Data Source=CinemaDB;Initial Catalog=Cinema;User Id=readonly;Password=readonly;"
			cnnCSentry_RuntimeUserName = "readonly"
			cnnCSentry_RuntimePassword = "readonly"

			' 
			' Directories
			'				
			gblBannerdir = "D:\websites\adsource\banners"
			gblShowtimedir = "D:\websites\adsource\viewandapprove" ' should be times-to-cs
			gblFromcsdir = "D:\websites\adsource\times-from-cs"
			gblDisplaydir = "D:\websites\adsource\display"
			gblBillingdir = "D:\websites\adsource\billing"
	...
	gblBg = "#3399cc" ' "#0D5180" '"#DFDFF7" ' "#3399cc"
	gblFg = "#FFFFFF"
	LVL_SUPER = 0		' God-like functions. Passwords and change chain.
	LVL_ADMIN = 1		' All functions. Banner ad scheduling.
	LVL_CENTRAL = 2		' All functions except banner sked.
	LVL_DISTRICT = 3	' Multi theater
	LVL_MGR = 7			' Either Exads or CinemaSource 
	LVL_CSX = 7			' csXpress User
	LVL_CSMGR = 8		' CinemaSource Theater functions only
	LVL_ADMGR = 9		' ExhibitorAds Theater functions only
	LVL_PAPER = 10		' Newspaper functions only
	LVL_FRM = 11		' FRM Reports only
	'
	' EXHIBITOR CONSTANTS for certain features
	'
	EX_TRANSLUX = "1"
	EX_CLEARVIEW = "111"
	EX_PACIFIC = "272"
	...
	' DEBUG
	'
	'for each ev in Request.ServerVariablesstrDate
	'	Response.Write ev & "=" & Request.ServerVariables(ev) & "<BR>"
	'next
	'Response.Write "Database: " + cnnCSentry_ConnectionString + "<BR>"
	'Response.End
' END OF INIT
