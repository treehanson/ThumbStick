...
require_once "db/db_conn.php";
	...
	//foreach ($houses as $house) {
		...
		$query = $db->prepare("
			...
			FROM Screens WITH(NOLOCK), Movies WITH(NOLOCK) WHERE ((house_id = 9826) or (house_id = 10367) or (house_id = 10792) or (house_id = 20453) or (house_id = 20454) or (house_id = 20455) or (house_id = 20456) or (house_id = 20457)) AND showdate = :date AND Screens.movie_id = Movies.movie_id
				...
				<div id="right">
					<div class="big-box round" style="padding-bottom: 0">
						<div class="slider-wrapper theme-default">
                            ...
                            <?php
							$query = $db->prepare("select * from websites.dbo.carousel where SiteID = 161 and category like '%home%' order by sort");
							$query->execute();
							$rows = $query->fetchAll();
					...
					<div class="big-box round">
						...
						<div id="scroller">
							...
							$query = $db->prepare("
select distinct screens.movie_id,movies.release, movies.name
from movies WITH(NOLOCK), houses WITH(NOLOCK), screens WITH(NOLOCK)
where movies.movie_id = screens.movie_id and screens.house_id = houses.house_id and screens.showdate = :date and houses.chain_id =1058 and movies.parent_id =0 order by movies.release desc, movies.name");			
						
						}elseif ($_GET['search'] == 'comingsoon'){
						$query = $db->prepare("
select distinct Mov.movie_id, mov.photos, Mov.actor1, Mov.actor2,  case when rtrim(isnull(cs.title,'')) = '' then Mov.name else cs.title end as name, Mov.mpaa, Mov.advisory, Mov.runtime, case when rtrim(isnull(cs.release,'')) = '' then Mov.release else cs.release end as rel, Mov.reltxt, Mov.release2, Mov.reltxt2, Mov.release3, Mov.reltxt3,  case when rtrim(isnull(cs.url, '')) = '' then Mov.url else cs.url end as movieurl, Rev.movie_id, Rev.capsule, Rev.sid, cs.start_date, cs.end_date, cs.release, cs.title, cs.url, cs.cs_type FROM cinema..movies Mov WITH(NOLOCK) INNER JOIN websites..comingsoon cs WITH(NOLOCK) ON Mov.movie_id = cs.movie_id INNER JOIN cinema..reviews Rev WITH(NOLOCK) ON rev.movie_id = cs.movie_id WHERE cs.siteid = 161 and rev.sid = 'HOME' and CAST(case when rtrim(isnull(cs.release,'')) = '' then Mov.release else cs.release end as DateTime) >  :date AND Mov.PARENT_ID = 0 order by rel, name
");
...
ORDER BY release DESC");
								...
								$query = $db->prepare("select distinct screens.movie_id,movies.release, movies.name
from movies WITH(NOLOCK), houses WITH(NOLOCK), screens WITH(NOLOCK)
where movies.movie_id = screens.movie_id and screens.house_id = houses.house_id and screens.showdate = :date and houses.chain_id =1058 and movies.parent_id =0 order by movies.release desc, movies.name");
						...
						$nsEvents .= "<a href='../movie_details.php?movie_id=" . $row['movie_id'] . "'><img src='" . getPoster($row['movie_id']) . "' alt='" . $row['name'] . "' title='" . getName(trim(str_replace("'","",$row['name']))) . "'/></a>";
